...
| TD-09 | P2 | Thin far-right Mac render edge | **Observed report; source boundary confirmed; visual gate blocked** | Mobile masks one trailing SwiftUI point after its Metal canvas; Mac does not. Computer Use lost window visibility before Dam/Surface, although bounded logs proved both current and exact accepted `7c78b72f...` controls kept presenting. No gameplay strip measurement or mask acceptance exists. Changing RT64 sizing remains rejected | Use a capture route that remains visible through menu interaction, capture fixed unchanged Dam/Surface frames, quantify the strip, then test the independent one-point host mask without touching drawable/viewport contracts |
| TD-10 | P2 | Stage-specific geometry, sky, water, and framebuffer-effect gaps | **Classified by subsystem; most reports still unreduced** | Active `skyRenderTri`/`skyRenderFull` produce viewport-scoped fog-color fills. The full textured sky/water candidate and its only `IsWater` path are disabled under `#if 0`; upstream also declares custom sky/water microcode unsupported. Frigate flat water and non-textured sky are known omissions. Glass/monitor effects and missing/clipped geometry remain unclassified reports | Treat known sky/water gaps as upstream feature work. For every other claim, require one stage/camera/settings/original-reference case before renderer code |
...
| TD-09 | `Sources/Mac/GoldenPadMacApp.swift`; mobile host mask in `Sources/RecompPrototypeApp.swift` | Source ownership confirmed; both Mac controls kept presenting after Computer Use lost visibility, but no gameplay capture exists | Fixed-camera Dam/Surface before/after captures with no other pixel-region regression; no RT64/window-size change |
| TD-10 | `ref/goldeneye64recomp/patches/skybox.c`; generated patch registry; RT64 effect support | Only fog-fill sky helpers active; full sky/water port disabled; remaining reports unreduced | TD-10 matrix in [`TESTING.md`](TESTING.md#td-10-stageeffect-classification-gate) |
...
### TD-10 stage/effect classification

| Class | Current status | What is established | Required next evidence |
| --- | --- | --- | --- |
| Textured sky/clouds | **Known approximation** | Active patched triangle/full helpers discard textured geometry and fill the active viewport with environment fog color | Fixed original-versus-GoldenPad scene only if prioritizing a complete upstream sky implementation |
| Frigate water | **Known upstream omission** | The only `IsWater`/`WaterImageId` textured path is inside disabled full-sky code; pinned upstream documents flat water/custom-microcode absence | Do not open a generic geometry bug. Scope a complete sky/water command implementation with original Frigate references |
| Glass, monitors, framebuffer-dependent effects | **Unvalidated** | Listed as open effect coverage; no exact current reproduction is recorded | Stage, room/camera, expected effect, Native N64/MSAA-off capture, and original reference |
| Missing/clipped stage geometry | **Unreduced report** | No evidence ties these reports to sky/water, culling, portals, scaling, or RT64 | One report per deterministic stage/position/yaw/pitch/settings tuple and original comparison |

Do not combine these classes into one renderer patch. A solid sky or flat water
is not proof that unrelated room geometry is missing, and a stage-local symptom
does not justify enabling the disabled 1,100-line sky candidate wholesale.
...
The 2026-08-22 TD-06 runtime discriminator rebuilt the pinned RT64 iPhoneOS and
Simulator archive closures and counted every depth `formatChanged` event by
width, size, and RDRAM-overlap cause. A single-player/menu control reached 1,437
display lists with zero events. A real four-player Temple match showed four
live quadrants, advanced from 6,035 to 7,348 display lists, and also recorded
