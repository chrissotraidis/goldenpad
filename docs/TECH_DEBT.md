...
| TD-09 | P2 | Thin far-right Mac render edge | **Observed report; source boundary confirmed; visual gate blocked** | Mobile masks one trailing SwiftUI point after its Metal canvas; Mac does not. Both the current build and exact accepted `7c78b72f...` control lost their accessible window before Dam/Surface, so no gameplay strip measurement or mask acceptance exists. Changing RT64 sizing remains rejected | Recover unchanged accepted Mac gameplay, capture fixed Dam/Surface frames, quantify the strip, then test the independent one-point host mask without touching drawable/viewport contracts |
...
| TD-09 | `Sources/Mac/GoldenPadMacApp.swift`; mobile host mask in `Sources/RecompPrototypeApp.swift` | Source ownership confirmed; exact accepted `7c78b72f...` control blocked before gameplay capture | Fixed-camera Dam/Surface before/after captures with no other pixel-region regression; no RT64/window-size change |
