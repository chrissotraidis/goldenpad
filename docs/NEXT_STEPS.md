...
| 6 | **TD-09 healthy Mac baseline + fixed edge captures** | Source ownership is confirmed, but both current and exact accepted Mac controls lost their accessible window before Dam/Surface. Reproduce and measure the strip first; only then test the independent one-point host mask. Never change RT64/window sizing for this defect. |
